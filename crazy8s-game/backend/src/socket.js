...
const initSocket = (server) => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('disconnect', (reason) => {
            ...
            if (session) {
                ...
                if (socket.gameId) {
                    ...
                    // Handle preparation phase vote recalculation if needed
                    try {
                        const Game = require('./models/game');
                        const game = Game.findById(socket.gameId);
                        
                        if (game && game.gameState === 'preparation') {
                            // Update player disconnected status in game
                            const player = game.getPlayerById(socket.id);
                            if (player) {
                                player.isConnected = false;
                                logger.info(`Player ${socket.playerName} disconnected during preparation phase in game ${socket.gameId}`);
                                
                                // Emit preparation phase update with updated vote counts
                                const preparationStatus = game.getPreparationStatus();
                                if (preparationStatus.inPreparation) {
                                    io.to(socket.gameId).emit('preparationPhaseUpdated', {
                                        gameId: socket.gameId,
                                        preparation: game.getGameState().preparation,
                                        votes: preparationStatus.votes,
                                        totalPlayers: preparationStatus.totalPlayers,
                                        disconnectedPlayer: socket.playerName,
                                        message: `${preparationStatus.votes} of ${preparationStatus.totalPlayers} connected players voted to skip preparation`
                                    });
                                }
                            }
                        }
                    } catch (error) {
                        logger.error(`Error handling preparation phase disconnect for socket ${socket.id}:`, error);
                    }
                }
            } else {
                ...
                if (reconnectionData && reconnectionData.success) {
                    ...
                    if (newSessionId) {
                        ...
                        if (!socket.disconnected) {
                            ...
                            // Handle preparation phase reconnection
                            try {
                                const Game = require('./models/game');
                                const game = Game.findById(reconnectionData.gameId);
                                
                                if (game && game.gameState === 'preparation') {
                                    // Update player connected status in game
                                    const player = game.getPlayerById(socket.id);
                                    if (player) {
                                        player.isConnected = true;
                                        logger.info(`Player ${reconnectionData.playerName} reconnected during preparation phase in game ${reconnectionData.gameId}`);
                                        
                                        // Emit preparation phase update with updated vote counts
                                        const preparationStatus = game.getPreparationStatus();
                                        if (preparationStatus.inPreparation) {
                                            io.to(reconnectionData.gameId).emit('preparationPhaseUpdated', {
                                                gameId: reconnectionData.gameId,
                                                preparation: game.getGameState().preparation,
                                                votes: preparationStatus.votes,
                                                totalPlayers: preparationStatus.totalPlayers,
                                                reconnectedPlayer: reconnectionData.playerName,
                                                message: `${preparationStatus.votes} of ${preparationStatus.totalPlayers} connected players voted to skip preparation`
                                            });
                                        }
                                    }
                                ...
                                logger.error(`Error handling preparation phase reconnection for socket ${socket.id}:`, error);
                            }
                        }
                    } else {
                ...
                if (!socket.disconnected) {
                    ...
                    try {
                        ...
                        if (game && game.gameState === 'preparation') {
                            ...
                            if (player) {
                                player.isConnected = true;
                                logger.info(`Player ${playerName} manually reconnected during preparation phase in game ${gameId}`);
                        ...
                        logger.error(`Error handling preparation phase manual reconnection for socket ${socket.id}:`, error);
                    }
                }
        ...
        // Handle skip preparation phase voting
        socket.on('skipPreparation', (data) => {
            try {
                if (!socket.gameId) {
                    socket.emit('error', { 
                ...
                // Validate game ID matches socket context
                ...
                // Vote to skip preparation
                const result = game.voteSkipPreparation(socket.id);
                
                if (result.success) {
                    logger.info(`Player ${socket.playerName} voted to skip preparation in game ${socket.gameId}`);
                    
                    // Get updated game state
                    const gameState = game.getGameState();
                    
                    if (result.transitioned) {
                        // All players voted - emit preparation ended
                        io.to(socket.gameId).emit('preparationPhaseEnded', {
                            gameId: socket.gameId,
                            gameState: gameState,
                        ...
                        logger.info(`Preparation phase ended by unanimous vote in game ${socket.gameId}`);
                    } else {
                    ...
                    socket.emit('skipVoteFailed', {
                        success: false,
                        error: result.message,
                        code: 'VOTE_FAILED'
                    });
                    
                    logger.warn(`Skip preparation vote failed for ${socket.playerName} in game ${socket.gameId}: ${result.message}`);
                }
            } catch (error) {
                logger.error(`Error handling skipPreparation event for socket ${socket.id}:`, error);
                socket.emit('error', {
                    message: 'Internal server error while processing skip vote',
                    code: 'INTERNAL_ERROR'
                });
            }
        });

        // Handle remove skip preparation vote
        socket.on('removeSkipVote', (data) => {
            try {
                if (!socket.gameId) {
                    socket.emit('error', { 
                        message: 'Not in a game',
                ...
                // Remove skip vote
                const result = game.removeSkipPreparationVote(socket.id);
                
                if (result.success) {
                    logger.info(`Player ${socket.playerName} removed skip vote in game ${socket.gameId}`);
                    
                    // Get updated game state and emit vote update to all players
                    const gameState = game.getGameState();
                    io.to(socket.gameId).emit('preparationPhaseUpdated', {
                        gameId: socket.gameId,
                        preparation: gameState.preparation,
                    ...
                    // Confirm removal to player
                    socket.emit('skipVoteRemoved', {
                        success: true,
                        votes: result.votes,
                        totalPlayers: result.totalPlayers
                    });
                } else {
                    socket.emit('skipVoteRemovalFailed', {
                        success: false,
                        error: result.message,
                        code: 'REMOVAL_FAILED'
                    });
                    
                    logger.warn(`Remove skip vote failed for ${socket.playerName} in game ${socket.gameId}: ${result.message}`);
                }
            } catch (error) {
                logger.error(`Error handling removeSkipVote event for socket ${socket.id}:`, error);
                socket.emit('error', {
                    message: 'Internal server error while removing skip vote',
                    code: 'INTERNAL_ERROR'
                });
            }
        });

        // Handle preparation status request
        socket.on('getPreparationStatus', (data) => {
            try {
                if (!socket.gameId) {
                    socket.emit('error', { 
                        message: 'Not in a game',
                ...
                socket.emit('preparationStatus', {
                    gameId: socket.gameId,
                    preparationStatus: status
                });
            } catch (error) {
                logger.error(`Error handling getPreparationStatus event for socket ${socket.id}:`, error);
                socket.emit('error', {
                    message: 'Internal server error while getting preparation status',
                    code: 'INTERNAL_ERROR'
                });
            }
        });
